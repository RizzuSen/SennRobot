...
class KyyTelegramHandler:
    ...
    def command(
        ...
        def _command(func):
            try:
                ...
                LOGGER.debug(f"[KYYROBOTCMD] Loaded handler {command} for function {func.__name__} in group {group}")
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                ...
                LOGGER.debug(f"[KYYROBOTCMD] Loaded handler {command} for function {func.__name__}")
    ...
    def message(self, pattern: Optional[str] = None, can_disable: bool = True, run_async: bool = True, group: Optional[Union[int, str]] = 60, friendly = None):
        def _message(func):
            try:
                ...
                LOGGER.debug(f"[KYYROBOTMSG] Loaded filter pattern {pattern} for function {func.__name__} in group {group}")
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                ...
                LOGGER.debug(f"[KYYROBOTMSG] Loaded filter pattern {pattern} for function {func.__name__}")
    ...
    def callbackquery(self, pattern: str = None, run_async: bool = True):
        def _callbackquery(func):
            ...
            LOGGER.debug(f'[KYYROBOTCALLBACK] Loaded callbackquery handler with pattern {pattern} for function {func.__name__}')
            return func
...
kyycallback = KyyTelegramHandler(k).callbackquery
kyyinline = KyyTelegramHandler(k).inlinequery
